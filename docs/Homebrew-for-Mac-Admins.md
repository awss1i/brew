...
/usr/bin/sudo -H -u "${install_user}" /usr/bin/env NONINTERACTIVE=1 \
  /bin/bash -s -- < /path/to/install.sh
```

The installer runs as the target account with its home directory, so its files and cache belong to that user.
